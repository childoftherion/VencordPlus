...
const settings = definePluginSettings({
    friendDirectMessages: {
        ...
        description:
            "Receive notifications in your friends' DMs when they join a voice channel",
        default: true,
    },
    friendDirectMessagesShowMembers: {
        type: OptionType.BOOLEAN,
        description:
            "Show a list of other members in the voice channel when receiving a DM notification of your friend joining a voice channel",
        default: true,
    },
    friendDirectMessagesShowMemberCount: {
        type: OptionType.BOOLEAN,
        description:
            "Show the count of other members in the voice channel when receiving a DM notification of your friend joining a voice channel",
        default: false,
    },
    friendDirectMessagesSelf: {
        type: OptionType.BOOLEAN,
        description:
            "Receive notifications in your friends' DMs, even if you are in the same voice channel as them",
        default: false,
    },
    ...
    allowedFriends: {
        ...
        description:
            "Comma or space separated list of friends' user IDs you want to receive join messages from",
        default: "",
    },
    ignoreBlockedUsers: {
        type: OptionType.BOOLEAN,
        description:
            "Do not send messages about blocked users joining voice channels",
        default: true,
    },
});
...
function sendVoiceStatusMessage(
    channelId: string,
    content: string,
    userId: string
): Message | null {
    if (!channelId) return null;
    const message: Message = createBotMessage({
        channelId,
        content,
        embeds: [],
    });
...
function isFriendAllowlisted(friendId: string) {
    ...
    const list = settings.store.allowedFriends
        .split(",")
        .join(" ")
        .split(" ")
        .filter((i) => i.length > 0);
...
export default definePlugin({
    ...
    description:
        "Receive client-side ephemeral messages when your friends join voice channels",
    ...
    flux: {
        VOICE_STATE_UPDATES({ voiceStates }: { voiceStates: VoiceState[] }) {
            ...
            for (const state of voiceStates) {
                ...
                if (
                    settings.store.ignoreBlockedUsers &&
                ...
                if (
                    ...
                    if (
                        ...
                        const sortedVoiceStates: User[] = Object.values(
                            voiceState as { [key: string]: VoiceState }
                        )
                            .filter((voiceState: VoiceState) => {
                                voiceState.user &&
                                    voiceState.user.id !== userId;
                            })
                        ...
                        if (otherMembersCount <= 0) {
                            memberListContent +=
                                ", nobody else is in the voice channel";
                        } else if (
                            settings.store.friendDirectMessagesShowMemberCount
                        ) {
                            memberListContent += ` with ${otherMembersCount} other member${
                                otherMembersCount === 1 ? "s" : ""
                            }`;
                        }
                        if (
                            settings.store.friendDirectMessagesShowMembers &&
                            ...
                            memberListContent += settings.store
                                .friendDirectMessagesShowMemberCount
                                ? ", "
                                : " with ";
                    ...
                    if (
                        dmChannelId &&
                        (selfInChannel
                            ? settings.store.friendDirectMessagesSelf
                            : true)
                    )
                        sendVoiceStatusMessage(
                            dmChannelId,
                            `has joined voice channel <#${channelId}>${memberListContent}`,
                            userId
                        );
                }
            }
