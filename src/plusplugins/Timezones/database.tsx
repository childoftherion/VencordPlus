...
export async function loadDatabaseTimezones(): Promise<boolean> {
    try {
        ...
        console.error("Failed to fetch time zones list:", e);
        return false;
    }
}
...
async function setTimezoneInternal(timezone: string): Promise<boolean> {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            showToast(error.message || "Failed to set time zone", Toasts.Type.FAILURE);
            return false;
        }

        showToast("Time zone updated successfully!", Toasts.Type.SUCCESS);
        return true;
    } catch (e) {
        console.error("Error setting time zone:", e);
        showToast("Failed to set time zone", Toasts.Type.FAILURE);
        return false;
    }
}
...
export async function deleteTimezone(): Promise<boolean> {
    ...
    if (!isAuthenticated) {
        showToast("You must be logged in to delete your time zone", Toasts.Type.FAILURE);
        return false;
    }
    ...
    try {
        ...
        if (!res.ok) {
            const error = await res.json().catch(() => ({ message: "Unknown error" }));
            showToast(error.message || "Failed to delete time zone", Toasts.Type.FAILURE);
            return false;
        }

        showToast("Time zone deleted successfully!", Toasts.Type.SUCCESS);
        return true;
    } catch (e) {
        console.error("Error deleting time zone:", e);
        showToast("Failed to delete time zone", Toasts.Type.FAILURE);
        return false;
    }
}
