...
async function setTimezoneInternal(timezone: string): Promise<boolean> {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            handleApiError(error, "Failed to set time zone");
            return false;
        }
        ...
        handleApiError(e, "Failed to set time zone");
        return false;
    }
}
...
async function deleteTimezoneInternal(): Promise<boolean> {
    try {
        ...
        if (!res.ok) {
            const error = await safeJsonParse(res);
            handleApiError(error, "Failed to delete time zone");
            return false;
        }
        ...
        handleApiError(e, "Failed to delete time zone");
        return false;
    }
}
